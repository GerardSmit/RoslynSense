...
public static class ConfigSchema
{
    ...
    private static readonly Dictionary<string, (string Title, string Description)> s_descriptions =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["tools.routes"] = ("HTTP routes", "Lists what the solution serves over HTTP, and where each endpoint is handled."),
            ["tools.templates"] = ("Templates", "Lists the screens an application declares in template files, as the tree they describe."),
            ...
            ["routes"] = ("HTTP routes", "What the solution serves over HTTP, listed under Routes: the path, the verb and the method that handles it. ASP.NET Core's attributes and minimal APIs and ASP.NET Web API's attributes are recognised already, so this section is only needed for a routing layer of your own \u2014 the `[Endpoint(\"orders\")]` that derives from nothing and that no list of framework names could have guessed at."),
            ["routes.attributes"] = ("Attributes", "The attributes of your own that carry a route template. Matched by name, so an attribute named here is read wherever it is declared."),
            ["routes.attributes[]"] = ("Attribute", "One attribute that carries a route template."),
            ["routes.attributes[].attributeName"] = ("Attribute", "The attribute's name, written with or without the `Attribute` suffix \u2014 both spellings mean the same attribute."),
            ["routes.attributes[].containingType"] = ("Class", "The full name of the attribute class. Leave empty to match the name wherever it is declared, which is usually what is wanted: naming the attribute is the reason for the entry."),
            ["routes.attributes[].pathIndex"] = ("Path is argument", "Which constructor argument carries the template, counted from 0. Leave empty for the first argument that is a string."),
            ["routes.attributes[].verb"] = ("HTTP method", "`GET`, `POST`, and so on. Leave empty when the attribute constrains none, which is what a bare route attribute does \u2014 an endpoint reachable by every method is a real thing and the list says so by leaving the column empty."),
            ["routes.methods"] = ("Methods", "The methods of your own that register an endpoint \u2014 a `MapEndpoint(path, handler)` wrapper, say."),
            ["routes.methods[]"] = ("Method", "One method that registers an endpoint."),
            ["routes.methods[].memberName"] = ("Member", "The method name."),
            ["routes.methods[].containingType"] = ("Class", "The full name of the class or interface declaring the member. Leave empty to match the member on any class."),
            ["routes.methods[].parameterTypes"] = ("Parameters", "One type name per parameter, `*` for a parameter of any type. Leave empty to match every overload."),
            ["routes.methods[].pathIndex"] = ("Path is parameter", "Which parameter carries the template, counted from 0. Leave empty for the first string."),
            ["routes.methods[].handlerIndex"] = ("Handler is parameter", "Which parameter says what runs, counted from 0. Used to point the Implementation button at it."),
            ["routes.methods[].verb"] = ("HTTP method", "`GET`, `POST`, and so on. Leave empty when the call constrains none."),
            ["routes.methods[].kind"] = ("The call", "`endpoint`, or `group` for one that opens a prefix every endpoint registered on its result carries. Leave empty for `endpoint`."),
            ["templates"] = ("Templates", "An application whose screens are declared in YAML rather than built in code has a structure \u2014 pages under pages, each hosting something that renders it \u2014 that exists in no file tree, because the files are named after the change that introduced them rather than after the screen they describe. Listed under Templates, with a button to the declaration and one to the control that renders it."),
            ["templates.folders"] = ("Folders", "Where the template files are, relative to a project. `App_Data/Templates` and `App_Data/TemplatesCustom` are read already, so this is only needed for a layout that keeps them somewhere else."),
            ["templates.controlFolders"] = ("Module folders", "Where the modules an application installed live, searched for the control that renders a module the templates name but never describe. `DesktopModules` is looked in already."),
            ["templates.locale"] = ("Language", "Which language tag a row's name is read from \u2014 `nl-NL`, `en-US`. Leave empty for whichever the file writes first. This is the customer's language rather than yours: the names in the tree should be the words in the screenshots you are sent."),
            ...
            ["debugger.enumerateResults"] = ("Enumerate results", "List a lazy enumerable's elements (a LINQ query, an iterator) when it is expanded, with the iterator's own state under Raw View. Off shows the iterator's members and puts the elements behind a Results View, as VS does. Expanding runs the sequence either way."),
