...
/// requests — the editing-surface handlers added after the original LSP batch.
/// </summary>
...
public class LspEditingFeatureTests
{
    ...
    public async Task SelectionRangeWidensFromTokenOutwards()
    {
        ...
        // Each step must contain the one inside it, and the chain must actually get wider —
    ...
    public async Task LinkedEditingCoversEveryUseOfAParameter()
    {
        ...
        // `a` in `Compute(int a, int b)` — declaration plus the two uses in the body.
